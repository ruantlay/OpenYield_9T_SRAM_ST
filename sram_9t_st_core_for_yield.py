...
#创建出各个管子，并定义其拓扑连接方式
        self.add_9T_cell(bl_node, blb_node, wl_node, q_node, qb_node,param_sweep)
...
##############################################################
class Sram9TCellForYield(Sram9TCell):#支持良率分析的6t sram单元
...
class Sram9TCoreForYield(Sram9TCore):
    ...
    def __init__(self, num_rows: int, num_cols: int,
                 pd_nmos_model_name: str, pu_pmos_model_name: str, pg_nmos_model_name: str,
                 model_dict: Dict[str, Dict[str, Any]], # 良率分析核心：传入 PDK 模型参数字典
                 pd_width=0.205e-6, pu_width=0.09e-6,
                 pg_width=0.135e-6, length=50e-9,
                 w_rc=False, pi_res=100 @ u_Ohm, pi_cap=0.001 @ u_pF,
                 param_sweep=False):

        # 在调用父类初始化前，先存储 model_dict
